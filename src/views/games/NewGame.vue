    ...
    <div>
        ...
        <form @submit.prevent="submitForm">
            ...
            <div class="form-group">
                ...
                <select id="plataform" v-model="form.plataform" :class="{ 'is-invalid': errors.plataform }">
                    <option v-for="plataform in plataforms" :key="plataform" :value="plataform">
                        {{ plataform }}
                    </option>
                </select>
                <div v-if="errors.plataform" class="invalid-feedback">{{ errors.plataform }}</div>
            </div>

            <div class="form-group">
                <label for="category">Categoría:</label>
                <select id="category" v-model="form.category" :class="{ 'is-invalid': errors.category }">
...
export default {
    ...
    methods: {
        ...
        getCategories() {
            const vm = this;
            this.axios.get("http://localhost:4000" + "/categories")
        ...
        save() {
            const vm = this;
            this.axios.post("http://localhost:4000" + "/games", {
                name: vm.form.name,
